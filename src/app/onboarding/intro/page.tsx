...
const EVIDENCE = [
  { tag: 'PRODUCTION EFFECT', title: '소리 내어 말하기', desc: '소리 내어 말한 정보는 눈으로만 읽은 정보보다 더 잘 기억되는 경향이 연구로 보고되었습니다.', source: '참고: MacLeod et al. (2010) — 산출 효과', chipBg: '#FBF0DA', chipColor: '#BA7517' },
  { tag: 'DUAL CODING THEORY', title: '보면서 말하기', desc: '언어와 시각 정보를 함께 사용하면 기억 형성에 도움이 될 수 있다는 인지심리학 이론입니다.', source: '참고: 이중부호화 이론 (Paivio)', chipBg: '#EAF2FB', chipColor: '#1E88E5' },
  { tag: 'SELF-AFFIRMATION', title: '나의 가치를 떠올리기', desc: '자신의 가치와 목표를 반복적으로 떠올리는 행동은 행동 변화와 심리적 안정에 도움을 줄 수 있습니다.', source: '참고: 자기확언 이론', chipBg: '#EEF4E6', chipColor: '#5E8A2E' },
  { tag: 'SELF-REFERENCE EFFECT', title: '내 얼굴을 마주하기', desc: '사람은 자신의 얼굴과 관련된 정보를 더 빠르고 강하게 처리하는 경향이 있습니다.', source: '참고: 자기참조 효과', chipBg: '#F3ECFB', chipColor: '#7A5AD9' },
]
  ...
  return (
    <div style={{
      ...
      {step === 0 && <HeroStep
        ...
        onEnded={handleEnded}
        onSkip={skipToOnboarding}
      />}
      {step === 1 && <MidlifeStep onNext={() => setStep(2)} onSkip={skipToOnboarding} />}
      {step === 2 && <HowItWorksStep onNext={() => setStep(3)} onSkip={skipToOnboarding} />}
      {step === 3 && <CtaStep onNext={() => setStep(4)} onSkip={skipToOnboarding} />}
      {step === 4 && <WhyItWorksStep onStart={skipToOnboarding} onSkip={skipToOnboarding} />}
    </div>
  )
}
...
function HeroStep({
  videoRef, words, phase, playLabel, onPlayTap, onTimeUpdate, onEnded, onSkip,
}: {
  videoRef: React.RefObject<HTMLVideoElement | null>
  words: { text: string; on: boolean }[]
  phase: DemoPhase
  playLabel: string
  onPlayTap: () => void
  onTimeUpdate: () => void
  ...
  return (
    <div style={{ minHeight: '100dvh', overflowY: 'auto', paddingBottom: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', padding: '2px 20px 12px' }}>
        <div style={{
          position: 'relative', width: '100%', maxWidth: 320, aspectRatio: '340 / 718',
...
/* ── Step 3: CTA card (content shortened) ──────────────────────── */
function CtaStep({ onNext, onSkip }: { onNext: () => void; onSkip: () => void }) {
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', background: CREAM }}>
      <button style={skipLink} onClick={onSkip}>건너뛰기</button>
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', padding: '64px 24px 24px' }}>
        <div style={{
          width: '100%', borderRadius: 24, padding: '36px 26px', textAlign: 'center',
          background: 'radial-gradient(ellipse 90% 120% at 50% 0%, #FBF0DA, #FFFCF8)',
          border: `1px solid ${LINE}`,
        }}>
          <h2 style={{ fontSize: 23, fontWeight: 800, letterSpacing: '-0.5px', lineHeight: 1.32, margin: '0 0 12px', color: INK }}>
...
/* ── Step 4: WHY IT WORKS (final) ──────────────────────────────── */
function WhyItWorksStep({ onStart, onSkip }: { onStart: () => void; onSkip: () => void }) {
  return (
